...
WHERE (SAFETY_CARDS.DEPT_ID In ('MWBF', 'MWBFMTCE')) AND (SAFETY_CARDS.EMPLOYEE_STATUS_CODE='01') AND (SAFETY_CARDS.EMPLOYEE_TYPE_CODE In ('B', 'C')) AND (SAFETY_CARDS.SUPERVISOR_IND='Y') AND (SAFETY_CARDS.MANAGER_TYPE_ID In (5, 8))
            ...
            try:
                if sm['SCHEDULED'][str(day)] == 2:
                    if sm['OBSERVATIONS'][str(day)] < 2:
                        # sm_line.append('&#10008')
                        if sm['TOOLBOX'][str(day)] > 0:
                            sm_line.append('&#10003&#10008')
                        else:
                            sm_line.append('&#10008&#10008')
                    else:
                        if sm['TOOLBOX'][str(day)] > 0:
                            sm_line.append('&#10003')
                        else:
                            sm_line.append('&#10008&#10003')
                        # sm_line.append('&#10003')
                else:
                    sm_line.append('&#9744')
            except:
...
# you = ["chris.schottmiller@aksteel.com", "JohnA.Huston@aksteel.com", "Craig.Budig@aksteel.com", "Doug.Stamper@aksteel.com", "John.Poirier@aksteel.com", "Robert.Johnson@aksteel.com", "Kris.Whitt@aksteel.com", "Matthew.Conaway@aksteel.com", "Dan.Brooke@aksteel.com", "Mike.Humphreys@aksteel.com", "Dimitries.Jenkins@aksteel.com", "Mike.Tannreuther@aksteel.com", "Jon.Wheeland@aksteel.com", "Ben.Wolf@aksteel.com", "Luke.McGlone@aksteel.com", "TomJ.George@aksteel.com", "Paul.Reed@aksteel.com"]
...
if len(next_weekly_lists) == 2:
    nl = []
    nl.extend(next_weekly_lists[0])
    ml = []
    ml.extend(next_weekly_lists[1])
    ml.pop(0)
    nl.extend(ml)

daily_alerts = list(set([item for sublist in daily_alerts_list for item in sublist]))
# if all([len(daily_alerts) > 0, datetime.datetime.today().isoweekday() in [3, 4, 5]]):
if (datetime.datetime.today().isoweekday() in [3, 4, 5]):
    #### SEND THE EMAIL
    import smtplib
    from email.mime.multipart import MIMEMultipart
    from email.mime.text import MIMEText
    ...
    me = "chris.schottmiller@aksteel.com"
    you = ["chris.schottmiller@aksteel.com"]
    # you = ["chris.schottmiller@aksteel.com", "Paul.Reed@aksteel.com", "JohnA.Huston@aksteel.com", "Craig.Budig@aksteel.com"]
    # Create message container - the correct MIME type is multipart/alternative.
    msg = MIMEMultipart('alternative')
