...
WHERE (SAFETY_CARDS.DEPT_ID In ('MWPA')) AND (SAFETY_CARDS.EMPLOYEE_STATUS_CODE='01') AND (SAFETY_CARDS.EMPLOYEE_TYPE_CODE In ('B', 'C'))  AND (SAFETY_CARDS.MANAGER_TYPE_ID In (5, 8))
...
AND ((SAFETY_CARDS.DEPT_ID In ('MWPA')) 
...
you_df = pd.read_excel(r'S:\QualityCI\Continuous Improvement\South Annealing\safety_email_lists\bof.xlsx', sheet_name='concast_mtc')
